...
(function( $ ) {
	...
	var methods = {
		init: function (options) {
			...
			$('.chat-window .chat_input').on('focus', function () {
				...
				$.ajax({
					...
					,success: function (resp) {
	                    ...
	                    +'</div>';
										...
										messageText = methods.formatMessage(resp.messages[i].text);
		...
		postMessage: function ($elem, channel) {
			...
			$.ajax({
				...
				,dataType: 'json'
				,data: {
					text: methods.formatMessage(message)
